...
> **用法**：任选一轮实现；做完勾选，并更新 [面试压力题速查-R1R2之后.md](../ms/Agent%20压力面试题完整分类.md) 底气列。

配套：[面试压力题速查](../ms/Agent%20压力面试题完整分类.md) · [需求总册](面试含金量优化需求文档.md) · [迭代学习索引](迭代学习索引.md)
...
| 3 | **R6 任务状态可恢复** | 第二节 #1–#4 | **已完成** |
| 3b | **R6b Postgres 双持久化** | 生产向 checkpoint/store | **已完成** |
| 4 | R4 安全沙箱 | #13 #14（#18 部分） | 待做 |
| 5 | R5 失败向 Eval | #17 | 待做 |
| 6 | R2b 轻量 run_id/用量 | #8；#9 #24 轻量版 | 待做 |
| 7 | R7 面试包装 | Demo/讲稿对齐；标明未做项 | 待做 |

建议顺序：R3 → R1b → R6 → R4 → R5 → R2b → R7。可跳着做，但 R5 最好在 R1b 之后（断言依赖门禁）。
...
- [x] **R6** 任务状态可恢复（见 [迭代-R6-任务状态可恢复学习指南.md](迭代-R6-任务状态可恢复学习指南.md)）
- [x] **R6b** TaskStore + Checkpointer 迁 PostgreSQL（docker-compose + AsyncPostgresSaver）
...
- [x] `api/task_store_postgres.py` + `AGENT_DATABASE_URL`
- [x] `AsyncPostgresSaver` + FastAPI lifespan
- [x] `docker-compose.yml` 本地 Postgres
- [x] memory 回退；单测 skipUnless PG
...
**挡题**：暂停/恢复？重启还在？连点幂等？多实例？

**关键落点**：

- [x] `api/task_store_postgres.py`：`PostgresTaskStore`；`AGENT_DATABASE_URL`
- [x] `agent/checkpointer.py`：`AsyncPostgresSaver`；memory 回退（单测）
- [x] `api/server.py`：同 thread pending/running → 409；`POST .../resume`
- [x] 前端：409 友好文案
- [x] 测试：postgres store / idempotency / checkpointer / resume
- [x] 文档：`迭代-R6-任务状态可恢复学习指南.md`；速查第二节 #1–#4 → ✅

**面试口径**：元数据 Postgres + 图 checkpoint Postgres；resume 是 turn 边界，非 tool 中途；多实例 Redis 口述。
...
| 能力 | 面试怎么说 |
|------|------------|
| 真暂停/恢复 | turn 边界 + checkpointer + resume API（R6）；无专用 pause / tool 中途 interrupt |
...
| 多实例 / Redis TaskStore | R6 单进程 Postgres；多实例需外置 store + sticky（口述） |
